    ...
    try {
      const res = await fetch("/api/generate/dry-run", {
        ...
        body: JSON.stringify({
          ...
          fast: true, // studio için hızlı mod
        }),
      });
  ...
  return (
    <main style={{ padding: 24, display: "grid", gap: 16 }}>
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        ...
        <button
          onClick={logout}
          style={{ marginLeft: "auto", padding: 8, border: "1px solid #ccc" }}
        >
          Çıkış
        </button>
      </div>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <input
            ...
            value={maxChars}
            onChange={(e) =>
              setMaxChars(e.target.value ? Number(e.target.value) : "")
            }
            placeholder="ör. 1200"
      ...
      <section style={{ display: "grid", gap: 8 }}>
        ...
        <textarea
          ...
          placeholder="Örn: 'TEC'de grev hazırlığı' vb."
          rows={5}
      ...
      {result && (
        <section style={{ display: "grid", gap: 12 }}>
          ...
          <div style={{ display: "grid", gap: 4 }}>
            ...
            <div>
              <strong>Meta Description:</strong> {result.metaDesc}
            </div>
            <div>
              <strong>Slug:</strong> {result.slug}
            </div>
            <div>
              <strong>Etiketler:</strong> {result.tags?.join(", ")}
            </div>
            <div>
          ...
          {Array.isArray(result.images) && result.images.length > 0 && (
            <div style={{ display: "grid", gap: 8 }}>
              ...
              <div style={{ fontSize: 12, color: "#666" }}>
                Not: Wikimedia Commons görselleri genelde{" "}
                <em>CC BY-SA</em> gibi lisanslarla gelir; bağlantıya tıklayıp
                lisans notunu kontrol ederek yayın sayfasında atfı koruyun.
                Unsplash/Pexels stok görsellerinde lisanslar uygundur; atıf
                vermek iyi pratiktir.
              </div>
            </div>
          )}
        </section>
