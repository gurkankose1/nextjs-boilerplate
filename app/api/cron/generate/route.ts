...
export async function GET(req: NextRequest){
  try{
    ...
    const q = await adminDb.collection("ingest_queue").where("status","==","pending").orderBy("createdAt","asc").limit(5).get();
    ...
    for(const doc of q.docs){
      ...
      try{
        ...
        const result = j.result as {
          seoTitle:string; metaDesc:string; slug:string; tags:string[]; keywords:string[]; html:string;
        ...
        // 4) Kuyruğu kapat
        await doc.ref.update({ status:"done", draftId: draftRef.id, processedAt: new Date().toISOString() });
        processed++;
      }catch(e:any){
        await doc.ref.update({ status:"error", error: String(e?.message||e), processedAt: new Date().toISOString() });
      }
    }
