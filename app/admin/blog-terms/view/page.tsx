...
export default async function AdminBlogTermViewPage({ searchParams }: PageProps) {
      ...
      <main className="min-h-screen bg-slate-950 text-slate-50">
        <div className="mx-auto w-full max-w-3xl px-4 py-10">
          <h1 className="mb-4 text-xl font-semibold">
            Geçersiz veya eksik ID parametresi
          </h1>
          <p className="text-sm text-slate-300 mb-3">
            URL&apos;de <code className="font-mono">?id=...</code> parametresi
            bulunamadı veya boş geldi. Liste sayfasından tekrar deneyebilirsin.
          </p>
          <div className="mt-4 rounded-xl border border-slate-800 bg-slate-900/70 p-3">
            <div className="text-xs text-slate-400 mb-1">
              searchParams (ham değer):
            </div>
...
{JSON.stringify(searchParams ?? {}, null, 2)}
  ...
  // 4) Veriyi şimdilik ham JSON olarak göster (debug modu)
  return (
    <main className="min-h-screen bg-slate-950 text-slate-50">
