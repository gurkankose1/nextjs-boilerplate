...
// Kategoriler (iç anahtar + ekranda görünen isim)
...
// Sanal editör isimleri (ileride köşe yazıları için de kullanacağız)
const CATEGORY_EDITOR_MAP: Record<string, string> = {
  ...
  "accidents": "Editör Ekibi", // buraya istersen sonra özel bir isim veririz
};
...
async function getLatestArticles(limit = 40): Promise<ArticleCard[]> {
  ...
  return snap.docs.map((doc) => {
    ...
    return {
      id: doc.id,
      title: (data.title as string) || (data.seoTitle as string) || "Havacılık Haberi",
...
export default async function HomePage({ searchParams }: HomePageProps) {
  ...
  // En çok okunanlar: views'e göre, eşitse tarihe göre (son çıkan üstte)
    ...
    <main className="min-h-screen bg-slate-950 text-slate-50 px-4 py-6 md:px-8 lg:px-16">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <header className="border-b border-slate-800 pb-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          {/* Ana navigasyon (ileride ayrı sayfalara gidecek) */}
        ...
        {/* İÇERİK ALANI: SOLDA HABERLER, SAĞDA EN ÇOK OKUNANLAR */}
        {articles.length === 0 ? (
                      ...
                      <li key={art.id} className="flex gap-2">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 text-[11px] text-slate-500">
                            {art.views !== undefined && (
                              <span>{(art.views ?? 0).toLocaleString("tr-TR")} okuma</span>
                            )}
                            {art.publishedAt && (
                              <span>
